...
**定时执行** (新功能，每5分钟监控一次):
```bash
# 前台运行 - 显示详细信息，按 Ctrl+C 停止
./start_monitor.sh

# 后台运行 - 更方便的守护进程模式
./start_daemon.sh start   # 启动后台监控
./start_daemon.sh status  # 查看运行状态
./start_daemon.sh stop    # 停止后台监控
./start_daemon.sh restart # 重启监控
```
...
- ⏰ **自动间隔**: 每5分钟执行一次监控
- 🔄 **持续运行**: 自动循环，无需手动重复执行
- 🛑 **优雅停止**: 支持 Ctrl+C 或命令停止
- 📝 **后台日志**: 守护进程模式支持日志文件记录
- 🔧 **进程管理**: 支持启动、停止、重启、状态查看

### 使用建议

- **测试阶段**: 使用前台模式 (`./start_monitor.sh`) 观察运行情况
- **生产环境**: 使用后台模式 (`./start_daemon.sh start`) 持续监控
- **服务器部署**: 推荐后台模式，便于远程管理
